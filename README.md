...
- **🌫️ 大気質情報**（黄砂・PM2.5・PM10の表示と注意喚起）
- **🌿 花粉情報**（花粉飛散レベルの表示と注意喚起）
- Open-Meteo / ウェザーニューズ ポールンロボ のデータを使用
...
- **総合判定**: 気温・体感温度・湿度・風速・降水状況・大気質を総合考慮
...
Open-Meteo Air Quality APIを利用して、黄砂・PM2.5・PM10の情報を取得・表示します。

### 黄砂レベル判定

| レベル | dust値 (μg/m³) | 表示 | ペナルティ |
|--------|----------------|------|-----------|
| 0 | 0-50 | なし | なし |
| 1 | 51-100 | 少ない | -5点 |
| 2 | 101-200 | やや多い | -15点 |
...
### PM2.5レベル判定（日本の環境基準に基づく）

| PM2.5 (μg/m³) | 状態 | ペナルティ | 警告 |
|---------------|------|-----------|------|
| 35以下 | 環境基準内 | なし | - |
| 36-50 | やや高め | -5点 | 敏感な方は注意 |
| 51-70 | 高い | -15点 | 長時間運動に注意 |
| 71以上 | 注意喚起レベル | -30点 | 激しい運動を避ける |

### 距離別ペナルティ倍率
長距離ほど呼吸量が増えるため、大気質の影響が大きくなります。
...
### 装備推奨
- **黄砂レベル2以上 または PM2.5が50超**: スポーツマスク
- **黄砂レベル3以上**: サングラス（目の保護）
...
花粉情報は**ウェザーニューズ「ポールンロボ」オープンデータ**を使用しています。

### 対応都市

組み込み都市（12都市）のみ対応。カスタム位置（`.runcast.conf`）では花粉情報は表示されません。

### 花粉レベル判定

| レベル | 個/cm² | 表示 | スコアペナルティ |
|--------|--------|------|----------------|
| 0 | 0 | なし | なし |
| 1 | 1-29 | 少ない | -5点 |
| 2 | 30-99 | やや多い | -15点 |
...
- 長距離ほど吸入量が増えるため、黄砂・PM2.5と同じ距離別倍率を適用
- レベル2以上でマスク着用推奨、レベル3以上でサングラスも推奨

### データ出典

花粉情報：[ウェザーニューズ ポールンロボ](https://wxtech.weathernews.com/pollen/index.html)
